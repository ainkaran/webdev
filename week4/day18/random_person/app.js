// Assignment: [Lab] Random Person
// Built an express.js application that picks a random name from a list of names separated by commas.
// Here is an example of the application: http://sinatra-apps.herokuapp.com/pick_random
...
create random form and functionalities
*/
...
app.get('/random', function (request, response) {
  response.render('random-person', {vrandomperson: undefined});
...
function randomPerson(str) {

let newArray = str.split(',');

let rname = newArray[getRandomIntInclusive(0, newArray.length-1)];
...
app.post('/random', function (request, response) {
  ...
  // console.log(request.body);
  let rp = randomPerson(request.body.info);
  response.render('random-person', {vrandomperson: rp});
});
